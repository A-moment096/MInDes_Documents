...
\begin{enumerate}
      ...
      \item 使用Powershell或命令提示符（cmd）, 输入\cverb|wsl --install|, 即可安装默认的Ubuntu发行版. 此过程中可能需要您提供若干次管理员权限. 随后您需要
      重启计算机以完成WSL\-Ubuntu的安装.
      \item 重启后会自动弹出Ubuntu的窗口(重启后若未启动, 则请于命令行中输入\cverb|wsl|或\cverb|ubuntu|), 此时会提示输入用户名和密码, 此处用户名应仅使用小写字母,
...
\begin{itemize}
      ...
      \item 挂载文件夹 (mnt): 该文件夹位于根目录\cverb|/|下. 在Linux中所有的磁盘均被抽象为文件夹存储于mnt中.
            因此, WSL中若要访问Windows系统中已有文件, 如路径为 \textbf{D:\textbackslash{}Work\textbackslash{}MInDes\textbackslash{}test.txt}
            的文件, 其在WSL中的路径则为 \textbf{/mnt/d/work/mindes/test.txt}. 其中, 第一个\cverb|/|即指
            根目录. 可以注意到linux系统文件路径不区分大小写.
      \item 用户文件夹 (User Directory): 指用户相关文件的存储路径, 类似于 Windows 中的 \textbf{C:\textbackslash{}User} 文件夹. 在Linux中,
...
为安装某个软件包, 此处以 G++ (Linux下的常用 C++ 编译器) 为例, 演示如何安装软件包.
\begin{enumerate}
      \setcounter{enumi}{-1}
...
为实现Linux系统下的C++开发环境配置, 需要安装以下的软件/程序包: G++, GDB, CMake. 同时为开发MInDes, 还需要安装运行库: \emph{fftw}, \emph{readline}.
...
如若采用VSCode + G++ + GDB + CMake的工具链模式进行Linux平台下的程序调试, 请按照如下步骤进行配置:
\begin{enumerate}
      \setcounter{enumi}{-1}
      \item 请确保在Windows下安装了VSCode与WSL, 且在Linux下正确安装了\\G++, GDB, CMake等软件工具. 为确认软件在Linux下已安装, 可输入\cverb|g++ --version|.
      ...
      \item 此步将为WSL上的VSCode安装若干插件. 同第1步中的操作, 安装插件{\sffamily{C++ Extension Pack}}.
\end{enumerate}
至此便可完成Linux下的环境配置.
...
\begin{enumerate}
      ...
      \item 通过命令行在您希望的位置创建一个文件夹, 如在用户文件夹下创建一个名为\textbf{test}的文件夹: \cverb|mkdir ~/test|,
      ...
      \item 此时VSCode将自动配置一个基础文件,名为\cverb|main.cpp|, 以及一个对应的CMake配置文件\cverb|CMakeLists.txt|, 且VSCode的CMake插件将调用CMake
            程序编译该文件, 结果将输出在名为\textbf{build}的文件夹中.
...
\end{enumerate}
若以上步骤均未出现问题, 说明Linux下以VSCode为宿主的G++ + GDB + CMake工具链已配置成功.
