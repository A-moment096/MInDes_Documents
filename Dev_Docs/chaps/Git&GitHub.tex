...
\begin{enumerate}
      ...
      \item 在弹出的对话框中, 为仓库起一个名字, 可选地填入描述性信息, 然后选择仓库的位置. 请注意, 这里的仓库位置是仓库文件夹所在的位置, 而不是仓库的路径.
            比如仓库名为\textbf{test}, 路径为\cverb|D:\Repositories|, 则创建的仓库文件夹的绝对路径为\cverb|D:\Repositories\test|.
...
以下是MInDes\_DEV的\textbf{main}分支中的内容.
\begin{itemize}
      \item \textbf{infile}: 内含可供MInDes读取使用的测试输入文件与示例输入文件.
      \item \textbf{lib}: 包含所有的在程序运行或编译过程中需要使用到的静态或动态链接库.
      \item \textbf{modules}: 包含程序中各个模块的源代码.
      \item \textbf{solvers}：包含程序中求解器主体的源代码．
...
\end{itemize}
以下是MInDes\_DEV的\textbf{develop}分支的额外内容.
\begin{itemize}
